        ...
        {TABS.map(tab => {
          if (tab.auth && !loggedIn) {
            return (
              <Link key={tab.to} to="/login"
                className="flex-1 flex flex-col items-center justify-center py-2 gap-0.5 text-gray-600 border border-gray-200 rounded-xl transition-colors min-h-[56px]">
          ...
          return (
            <Link key={tab.to} to={tab.to} data-testid={`bottom-nav-${tab.label.toLowerCase().replace(/\s+/g,"-")}`}
              className={`flex-1 flex flex-col items-center justify-center py-2 gap-0.5 rounded-xl border transition-colors min-h-[56px] relative ${
                active ? "text-red-700 border-red-200 bg-red-50" : "text-gray-600 border-gray-200"
              }`}
            >
              {active && <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-red-700 rounded-full" />}
              <tab.icon className="w-5 h-5" />
              <span className={`text-[10px] font-bold ${active ? "text-red-700" : "text-gray-700"}`}>{tab.label}</span>
            </Link>
          );
        })}
      </div>
