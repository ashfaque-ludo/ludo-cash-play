...
const COMMISSION_PCT = parseFloat(process.env.PLATFORM_COMMISSION_PCT || "10") / 100;
...
router.post("/api/upload", auth, upload.single("screenshot"), async (req, res) => {
  try {
    ...
    const prizeAmount = parseFloat(amount) || 0;
    const commission = prizeAmount * COMMISSION_PCT;
    const net_prize = prizeAmount - commission;
