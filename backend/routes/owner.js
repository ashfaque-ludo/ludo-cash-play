...
router.get("/stats/complete", async (req, res) => {
  try {
    ...
    const [
      ...
      Transaction.aggregate([{ $match: { type: "deposit", status: { $in: ["approved", "completed"] } } }, { $group: { _id: null, total: { $sum: "$amount" } } }]),
      Transaction.countDocuments({ type: "deposit", status: "pending" }),
      Transaction.countDocuments({ type: "deposit", status: { $in: ["approved", "completed"] } }),
